...
unsigned int dosageStepsCount = 0;
unsigned int stepperOrServo = 1;
			...
			timeSplit[0] = '\0';
				...
				if(tempHour == hours && tempMin == mins && tempSec == secs) //if an event matches current time
				{
					if(checkDrugInterval() == 0)
					{
						...
						dispenseActive = 0;
						stepperState = 1; //run the stepper motor.
...
void runEventWindow()
{
	...
	do
	{
		...
		if(in == 0x1 && switchFlag == 0 && stepperComplete == 1) //If switch is turned on and motor not currently running, start boost.
		{
			switchFlag = 1;
			validateBoost();
		}
		...
		{
			if(stepperComplete == 1) //If stepper not running search for events.
			{
				checkEvents();
			}
			display = 0;
		...
		if(((*scsr) & 0x20) != 0x0) //if key pressed.
        {
			...
			{
				while(stepperComplete == 0) //Wait for current event to end.
				{
					if (display == 1) 
					{
						display = 0;
