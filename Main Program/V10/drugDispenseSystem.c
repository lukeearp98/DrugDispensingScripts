...
void runServo();
/*	Globals */
char controllerName[30] = ""; //Global so does not reset when re-entering menus.
char drugName[30] = ""; //Global so does not reset when re-entering menus.
char currentDate[15] = ""; //Global so can be written to at start and then used in the logs.
char patientID[10] = ""; //Global so does not reset when re-entering menus.
char lastDispenseTime[15] = "00:00:00"; 
char dosageAmount[10] = "1"; //Global so does not reset when re-entering menus.
char boostsAvailable[5] = "5"; //Global so does not reset when re-entering menus.
char intervalTime[30] = "00:00:30"; //Global so does not reset when re-entering menus.
char adminPassword[30] = "admin"; 
char recentLogs[20][200]; //Global so does not reset everytime something is logged.
char drugEvents[11][15]; //Global so everytime enter an event menu it doesnt reset the events.
...
int checkDrugInterval()
{
	int lastDispenseTimeHrs, lastDispenseTimeMins, lastDispenseTimeSecs;
